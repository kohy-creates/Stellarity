...
execute as @n[type=#kohara:ammo,sort=nearest,tag=!stellarity.aware] positioned as @s run function stellarity:items/prismatic_punch/spawn_blast

data modify storage stellarity:temp prismatic_punch.item set from entity @s SelectedItem
execute store result score @n[type=marker,sort=nearest,tag=stellarity.prismatic_blast] stellarity.items.prismatic_punch.piercing run \
...
execute if data storage stellarity:temp prismatic_punch.item.components."minecraft:enchantments".levels."minecraft:multishot" run \
	tag @n[type=marker,sort=nearest,tag=stellarity.prismatic_blast] add stellarity.prismatic_blast.multishot
