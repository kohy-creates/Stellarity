...
# Piercing Cold
execute if data storage stellarity:temp {aery_sword:{item:{stellarity.aery_sword:{abilities:["piercing_cold"]}}}} as @n[type=!#kohara:invalid_targets,nbt={HurtTime:10s}] if predicate kohara:chance/33percent at @s run function stellarity:items/frigid_harvester/abilities/piercing_cold/apply
