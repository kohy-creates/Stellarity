execute positioned ~ ~-1 ~ as @n[type=item_display,sort=nearest,distance=..0.01,tag=!stellarity.aots] run tag @s add stellarity.aots
execute positioned ~ ~-1 ~ as @n[type=item_display,sort=nearest,distance=..0.01,tag=stellarity.aots] at @s run function stellarity:mechanics/altar_of_light/loop/item_display
