...
execute at @s unless dimension minecraft:the_end at @e[type=minecraft:ender_dragon,tag=!fe.boss,tag=!fe.ender_dragon,tag=!fe.init,tag=!fe.mob,predicate=stellarity:locations/dragons_den/in_biome,distance=..1000,limit=1] run function stellarity:post_gen/initialize
